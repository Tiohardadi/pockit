...
class EditTransaksi extends StatefulWidget {
  final int transactionId; // Parameter transactionId
...
class _EditTransaksiState extends State<EditTransaksi> {
  ...
  String? _kategoriId; // Variabel untuk menyimpan ID kategori
  ...
  Future<void> updateTransaction(AddTransactionEntities transaction, int balanceId) async {
    ...
    try {
      ...
      if (response.statusCode != 200) {
        throw Exception('Gagal memperbarui transaksi. Kode status: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> _updateTransaction() async {
    ...
    try {
      ...
      print('Sending updated transaction: ${transaction.toString()}');

      // Kirim data ke API
      await updateTransaction(transaction, widget.transactionId);
      ...
      _showErrorDialog('Error saat memperbarui transaksi: $e');
    }
  }
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  await deleteTransaction(widget.transactionId);
                  ...
                  // Tampilkan pesan sukses
                  _showSuccessDialog('Transaksi berhasil dihapus!');
                } catch (e) {
                  setState(() {
                    _isLoading = false;
                  });
                  print('Error deleting transaction: $e');
                  _showErrorDialog('Error saat menghapus transaksi: $e');
                }
              },
            ),
          ],
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 16),
                    Text("Editing Transaction: ${widget.transactionId}"),
                    SizedBox(height: 16),
                    Align(
                      ...
                      children: [
                        Expanded(
                          child: CustomButton(
                            ...
                            backgroundColor: Color(0xffF87272),
                          ),
                        ),
